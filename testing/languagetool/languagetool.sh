...
if $asked_for_help; then
  ...
  java -cp $CP $CLI_command -h | sed "s/java -jar languagetool-commandline.jar/languagetool/"
  echo
  echo "Graphical user interface (GUI) help:"
  java -cp $CP $GUI_command -h | sed "s/java org.languagetool.gui.Main/languagetool/"
  echo
  echo "HTTP server help:"
  java -cp $CP $NSRV_command -h | sed "s/HTTPServer/languagetool --http/"
  echo
  echo "HTTPS server help:"
  java -cp $CP $SSRV_command -h | sed "s/HTTPSServer/languagetool/"
else
  if $start_server; then
    if $http_server; then
      java -cp $CP $NSRV_command "${argv[@]}"
    else
      java -cp $CP $SSRV_command "${argv[@]}"
    fi
  else
    if $file_present; then
      java -cp $CP $CLI_command "${argv[@]}"
    else
      java -cp $CP $GUI_command "${argv[@]}"
    fi
  fi
fi
